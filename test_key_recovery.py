...
def convert_to_binary(l):#transformation
  n = len(l);
  k = WORD_SIZE * n;
...
def hw(v):
  # helper function to calculate hamming weight(used to measure the similarities in binary strings in this context: diff btw plaintexts and predicted plaintexts generated by trained model during key recivery attack)
...
def gen_key(nr):
  #The purpose of this function is to provide a random key that can be used to encrypt the plaintext-ciphertext pairs that are used as input to the neural network model during training and testing
  key = np.frombuffer(urandom(8),dtype=np.uint16);
  ks = speck.expand_key(key, nr);
  return(ks);

def gen_plain(n):
  #The purpose of this function is to provide a random plaintext message that can be used to encrypt with the secret key to produce the corresponding ciphertext.
...
def gen_challenge(n, nr, diff=(0x211, 0xa04), neutral_bits = [20,21,22,14,15,23], keyschedule='real'):
  #The gen_challenge function generates a set of plaintext-ciphertext pairs by generating a random secret key using the gen_key function and then encrypting a series of random plaintext messages using the secret key. The resulting plaintext-ciphertext pairs are stored in two separate arrays.
  pt0, pt1 = gen_plain(n);
  ...
  ct0a, ct1a = speck.encrypt((pt0a, pt1a), key);
  ct0b, ct1b = speck.encrypt((pt0b, pt1b), key);
  return([ct0a, ct1a, ct0b, ct1b], key);

def find_good(cts, key, nr=3, target_diff = (0x0040,0x0)):
  #utility function used to filter a set of plaintext-ciphertext pairs, select a subset of pairs that are suitable for use in the key recovery attack, and randomly sample a specified number of pairs from the filtered list
  pt0a, pt1a = speck.decrypt((cts[0], cts[1]), key[nr:]);
  pt0b, pt1b = speck.decrypt((cts[2], cts[3]), key[nr:]);
...
def verifier_search(cts, best_guess, use_n = 64, net = net6):
  ...
  pt0a, pt1a = speck.dec_one_round((ct0a, ct1a), ck1);
  print("plaintext:", pt0a)
  pt0b, pt1b = speck.dec_one_round((ct0b, ct1b), ck1);
  pt0a, pt1a = speck.dec_one_round((pt0a, pt1a), ck2);
  pt0b, pt1b = speck.dec_one_round((pt0b, pt1b), ck2);
  X = speck.convert_to_binary([pt0a, pt1a, pt0b, pt1b]); #XAI: Output
  # print("x", X)
  # j1 = np.array(X);
  # np.savetxt("x_value.csv", j1, delimiter=",")
  ...
  # print("z4", Z)
  # j1 = np.array(Z);
  # np.savetxt("z_veri_value .csv", j1, delimiter=",")
  # j2 = np.array(Y2);
  # j1_df = pd.DataFrame(j1) 
  # j2_df = pd.DataFrame(j2)
  # j1_df.to_csv("C:/Users/Lenovo/Desktop/3162/New folder/z_veri_value.csv", sep="\t", index=False, encoding='utf-8');
  ...
  # np.savetxt("keys1.csv", keys1, delimiter=",")
...
def wrong_key_decryption(n, diff=(0x0040,0x0), nr=7, net = net7):
  ...
  for i in range(2**16):
    ...
    ks = speck.expand_key(keys, nr+1); #ks[nr-1] = 17123;
    ...
    rsubkeys = i ^ ks[nr];
  #rsubkeys = rdiff ^ 0;
    c0a, c1a = speck.dec_one_round((ct0a, ct1a),rsubkeys);
    c0b, c1b = speck.dec_one_round((ct0b, ct1b),rsubkeys);
    X = speck.convert_to_binary([c0a, c1a, c0b, c1b]);
    # print("x1", X)
    j2 = np.array(X);
    # np.savetxt("x1_value.csv", j2, delimiter=",")
    Z = net.predict(X,batch_size=10000);
    Z = Z.flatten();
    means[i] = np.mean(Z);
    sig[i] = np.std(Z);
...
def bayesian_key_recovery(cts, net=net7, m = m7, s = s7, num_cand = 32, num_iter=5, seed = None):
  ...
  for i in range(num_iter):
    k = np.repeat(keys, n);
    c0a, c1a = speck.dec_one_round((ct0a, ct1a),k); c0b, c1b = speck.dec_one_round((ct0b, ct1b),k);
    X = speck.convert_to_binary([c0a, c1a, c0b, c1b]);
    print("x2", X)
    j3 = np.array(X);
    np.savetxt("x2_value.csv", j3, delimiter=",")
    Z = net.predict(X,batch_size=10000);
...
def test_bayes(cts,it=1, cutoff1=10, cutoff2=10, net=net7, net_help=net6, m_main=m7, m_help=m6, s_main=s7, s_help=s6, verify_breadth=None):
  ...
  for j in range(it):
      ...
      if (vtmp > cutoff1):
        ...
        for i2 in l2:
          c0a, c1a = speck.dec_one_round((cts[0][i],cts[1][i]),keys[i2]);
          c0b, c1b = speck.dec_one_round((cts[2][i],cts[3][i]),keys[i2]);         
