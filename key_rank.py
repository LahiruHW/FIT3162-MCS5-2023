...
def key_rank_one_round(nr, net, n_blocks=1, diff=(0x0040,0x0)):
  ...
  pt0a, pt1a = speck.dec_one_round((pt0a, pt1a),0);
  pt0b, pt1b = speck.dec_one_round((pt0b, pt1b),0);
  key = np.frombuffer(urandom(8),dtype=np.uint16);
  ks = speck.expand_key(key, nr); k1 = ks[nr-1];
  ct0a, ct1a = speck.encrypt((pt0a, pt1a),ks);
  ct0b, ct1b = speck.encrypt((pt0b, pt1b),ks);
  trial_keys = np.arange(2**16);
  c0a, c1a = speck.dec_one_round((ct0a, ct1a),trial_keys);
  c0b, c1b = speck.dec_one_round((ct0b, ct1b),trial_keys);
  ...
  X = speck.convert_to_binary([c0a.flatten(), c1a.flatten(), c0b.flatten(), c1b.flatten()]);
