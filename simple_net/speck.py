...
def real_differences_data(n, nr, diff=(0x0040,0)):
  ...
  keys = np.frombuffer(urandom(8*n),dtype=np.uint16).reshape(4,-1);
  #generate plaintexts
  plain0l = np.frombuffer(urandom(2*n),dtype=np.uint16);# randomly generated
  print("plaintext:" , plain0l)

  plain0r = np.frombuffer(urandom(2*n),dtype=np.uint16);# randomly generated 
  print("plaintextr:" , plain0r)
  #apply input difference
  plain1l = plain0l ^ diff[0]; plain1r = plain0r ^ diff[1]; # XOR PLAINTEXT
  print("pl1r", plain1r)
  print("pl1l", plain1l)
  num_rand_samples = np.sum(Y==0);
  ...
  print("cl0", ctdata0l)
  print("cr0", ctdata0r)
  plain = decrypt((ctdata0l,ctdata0r), ks)
  print("plainnn", plain)
...
print("round5", real_differences_data(10**6,5))