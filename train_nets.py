...
def cyclic_lr(num_epochs, high_lr, low_lr):
  # cyclically varying the learning rate of the optimizer during training
  res = lambda i: low_lr + ((num_epochs-1) - i % num_epochs)/(num_epochs-1) * (high_lr - low_lr);
  return(res);

def make_checkpoint(datei):
  # save the state of the neural network model and the optimizer to disk at regular interval during training
...
def train_speck_distinguisher(num_epochs, num_rounds=7, depth=1):
    ...
    X, Y = speck.make_speck_train_data(10**7,num_rounds);
    X_eval, Y_eval = speck.make_speck_train_data(10**6, num_rounds);
