...
def key_rank_one_round(nr, dist, n_blocks=1, diff=(0x0040,0x0)):
  ...
  ks = speck.ex_key_test(key, nr); k1 = ks[nr-1];
  ct0a, ct1a = speck.enc_test((pt0a, pt1a), ks);
  ct0b, ct1b = speck.enc_test((pt0b, pt1b), ks);
  trial_keys = np.arange(2**16);
  c0a, c1a = speck.dec_one_round((ct0a, ct1a), trial_keys); c0b, c1b = speck.dec_one_round((ct0b, ct1b), trial_keys);
